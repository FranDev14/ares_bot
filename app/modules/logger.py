...
from app.libs.constants import LOGGER_CHANNEL_ID

COLOR_LEVELS = {
    logging.INFO: Color.blurple(),
    logging.ERROR: Color.magenta,
    logging.WARNING: Color.gold(),
    logging.CRITICAL: Color.red(),
    logging.DEBUG: Color.fuchsia()
}

class DiscordHandler(logging.Handler):
    """
    A class that implement logging. This handler method send logs to Discord Channels.
    """
    ...
    def __init__(self, bot: commands.Bot, *args, **kwargs):
        ...
        self.client = bot
        self.log_channel =self.client.get_channel(int(LOGGER_CHANNEL_ID))

    def _level_to_color(self, level_number: int):
        return COLOR_LEVELS.get(level_number)
    
    def emit(self, record):
        if not self.client.loop.is_running():
            # If the loop is not running, discord not connected
            return
